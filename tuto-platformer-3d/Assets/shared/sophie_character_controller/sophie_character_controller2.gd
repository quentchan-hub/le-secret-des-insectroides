...
# Référence au nœud CamPivot (pour obtenir l'orientation de la caméra)
...
func _physics_process(delta: float) -> void:
	# Calcul de la hauteur du sol pour la caméra
	...
	# Séparation de la vitesse Y pour ne pas interpoler la gravité
	var y_velocity := velocity.y
	velocity.y = 0.0

	# Récupération de l'entrée et de l'état de mouvement
	var move_direction := _get_camera_oriented_input()
	var angle_to_move := velocity.signed_angle_to(move_direction, Vector3.UP)

	var is_just_jumping := Input.is_action_just_pressed("sophie_jump") and is_on_floor()
	var is_air_boosting := Input.is_action_pressed("sophie_jump") and not is_on_floor() and y_velocity > 0.0
	...
	# Compte les frames où le personnage est au sol
	if is_on_floor():
		_frames_on_floor += 1

	# Logique de mouvement horizontal
	if _is_animating:
		pass  # Le personnage ne peut pas bouger pendant les animations
	...
	# Restauration de la vitesse Y et calcul de la gravité
	var xz_velocity := Vector3(velocity.x, 0, velocity.z)
	velocity.y = y_velocity

	if _is_animating:
		pass  # La gravité n'affecte pas le personnage pendant les animations
	elif not is_on_floor():
		velocity += Vector3.DOWN * gravity * delta
	velocity.y = max(velocity.y, -terminal_velocity)

	# Vérification de l'état de saut
	if _is_animating:
		pass  # Pas de saut pendant les animations
	...
	if _is_animating:
		pass  # Ne pas tourner le personnage pendant les animations
...
func _get_camera_oriented_input() -> Vector3:
	...
	input = camPivot.global_transform.basis * input  # Utilise camPivot au lieu de camera_controller
	input.y = 0.0
...
func _orient_character_to_direction(direction: Vector3, delta: float) -> void:
	...
	if not direction.is_zero_approx():
		...
		_rotation_root.transform.basis = Basis(_rotation_root.transform.basis.get_rotation_quaternion().slerp(rotation_basis, min(1.0, delta * rotation_speed))).scaled(model_scale)
